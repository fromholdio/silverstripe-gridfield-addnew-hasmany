...
<div class="add-new-hasmany-search-results">
	<% if $Items %>
		<ul class="list-group add-new-hasmany-search-items" data-add-link="$Link('add')">
			<% loop $Items %>
				<li class="$EvenOdd list-group-item list-group-item-action">
                    <a href="#" data-id="$Result.ID" <% if $Up.RedirectToEdit %>data-edit-link="{$Up.Link('edit')}?id={$Result.ID}"<% end_if %>>$Title</a>
                </li>
			<% end_loop %>
		</ul>
	<% else %>
	...
	<% if $Items.MoreThanOnePage %>
		<ul class="pagination add-new-hasmany-search-pagination">
